        ...
        const body = {
          ...
          .then((resp ) => {
            if (resp) {    
              ...
              if (decode.type === 'OPERADOR' || decode.type === 'DIRECTOR'|| decode.type === 'OPERADOR_EDITOR' || decode.type === 'CONSULTOR')
              {
  ...
  login() {
    ...
    this.loginService.login(body).toPromise().then((response: any) => {
      if (response) {    
        ...
        else if (decode.type === 'OPERADOR' || decode.type === 'DIRECTOR' || decode.type === 'OPERADOR_EDITOR' || decode.type === 'CONSULTOR')
        {
          this.loggedIn = response!=null;
          this.router.navigate(['/panel', { outlets: { 'OutletAdmin': ['listarPat'] } }]);
        }
